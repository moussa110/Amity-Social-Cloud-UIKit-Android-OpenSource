...
class AmityMyFeedFragment : AmityFeedFragment() {
	...
	override fun getEmptyView(inflater: LayoutInflater): View {
		val binding = AmityViewMyTimelineFeedEmptyBinding.inflate(inflater,
	...
	override fun loadFeedAfterGetPinnedPost(pinnedPostListener: (AmityPost?) -> Unit) {
		AmityCoreClient.getCurrentUser().take(1).doOnNext { user ->
	...
	class Builder internal constructor() {
		...
		fun build(activity: AppCompatActivity): AmityMyFeedFragment {
			...
			fragment.mViewModel =
				ViewModelProvider(activity).get(AmityMyTimelineViewModel::class.java)
			...
			if (communityClickListener == null) {
				communityClickListener = object : AmityCommunityClickListener {
					override fun onClickCommunity(community: AmityCommunity) {
						AmitySocialUISettings.globalCommunityClickListener.onClickCommunity(fragment,
							community)
					}
				}
			}
