...
interface PostViewModel {
	...
	fun getPost(postId: String,
	            ...
	            onError: () -> Unit): Flowable<AmityPost> {
		return AmitySocialClient.newPostRepository().getPost(postId).subscribeOn(Schedulers.io())
	...
	fun addPostReaction(post: AmityPost, reaction: Reactions): Completable {
		return post.react().addReaction(reaction.reactName).subscribeOn(Schedulers.io())
			.observeOn(AndroidSchedulers.mainThread())
	}

	fun removePostReaction(post: AmityPost, reaction: Reactions): Completable {
		return post.react().removeReaction(reaction.reactName).subscribeOn(Schedulers.io())
			.observeOn(AndroidSchedulers.mainThread())
	}

	fun reportPost(post: AmityPost, onSuccess: () -> Unit, onError: () -> Unit): Completable {
		return post.report().flag().subscribeOn(Schedulers.io())
	...
	fun unReportPost(post: AmityPost, onSuccess: () -> Unit, onError: () -> Unit): Completable {
		return post.report().unflag().subscribeOn(Schedulers.io())
			.observeOn(AndroidSchedulers.mainThread()).doOnComplete {
				onSuccess.invoke()
			}.doOnError {
				onError.invoke()
	...
	fun deletePost(post: AmityPost, onSuccess: () -> Unit, onError: () -> Unit): Completable {
		return post.delete().subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread())
			.doOnComplete {
				onSuccess.invoke()
			}.doOnError {
				onError.invoke()
	...
	fun closePoll(pollId: String, onSuccess: () -> Unit, onError: () -> Unit): Completable {
		return AmitySocialClient.newPollRepository().closePoll(pollId = pollId)
	...
	fun approvePost(
		...
		return AmitySocialClient.newPostRepository().reviewPost(postId).approve()
	...
	fun declinePost(postId: String,
	                onAlreadyDeclined: () -> Unit,
	                onError: () -> Unit): Completable {
		return AmitySocialClient.newPostRepository().reviewPost(postId).decline()
			.subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).doOnError {
				if (AmityError.from(it) == AmityError.FORBIDDEN_ERROR) {
	...
	fun getSharingOptionMenuItems(
		...
		if (possibleTargets.contains(AmityPostSharingTarget.MyFeed) || (possibleTargets.contains(
				AmityPostSharingTarget.OriginFeed) && (((target as? AmityPost.Target.USER)?.getUser()
				?.getUserId() ?: "") == AmityCoreClient.getUserId()))) {
			items.add(myFeed)
		}

		if (possibleTargets.contains(AmityPostSharingTarget.PrivateCommunity) || possibleTargets.contains(
				AmityPostSharingTarget.PublicCommunity) || (possibleTargets.contains(
				AmityPostSharingTarget.OriginFeed) && (target is AmityPost.Target.COMMUNITY))) {
			items.add(groupFeed)
		}
	...
	fun getPostOptionMenuItems(post: AmityPost,
	                           editPost: () -> Unit,
	                           deletePost: () -> Unit,
	                           reportPost: () -> Unit,
	                           unReportPost: () -> Unit,
		...
		val pinPostMenuItem =
			BottomSheetMenuItem(null, null, R.string.amity_pin_post, pinPost ?: {})
		val unpinPollMenuItem =
			BottomSheetMenuItem(null, null, R.string.amity_unpin_poll, unpinPost ?: {})
		val unpinPostMenuItem =
			BottomSheetMenuItem(null, null, R.string.amity_unpin_post, unpinPost ?: {})
				...
				is AmityPost.Target.COMMUNITY -> {
					...
					val hasDeletePermission =
						AmityCoreClient.hasPermission(AmityPermission.DELETE_USER_FEED_POST)
							.atGlobal().check().blockingFirst(false)
					if (hasDeletePermission) {
						items.add(deletePostMenuItem)
					}
				}
	...
	fun vote(pollId: String, answerIds: List<String>): Completable {
		return AmitySocialClient.newPollRepository().vote(pollId = pollId, answerIds = answerIds)
	}
	...
	fun searchCommunityUsersMention(communityId: String,
		...
		return AmitySocialClient.newCommunityRepository().membership(communityId)
			.searchMembers(keyword).membershipFilter(listOf(AmityCommunityMembership.MEMBER))
			.build().query().subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread())
			.doOnNext {
				onResult.invoke(it)
			}.ignoreElements()
	}
