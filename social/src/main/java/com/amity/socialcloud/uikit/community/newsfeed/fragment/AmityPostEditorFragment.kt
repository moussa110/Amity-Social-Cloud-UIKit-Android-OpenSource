...
class AmityPostEditorFragment : AmityBaseCreatePostFragment() {
	...
	override fun isRightButtonActive(): Boolean {
		return if (isEditMode() && !viewModel.hasUpdateOnPost(binding.etPost.text.toString()
				.trim())) {
			false
		} else {
	...
	private fun updatePost() {
		...
		viewModel.updatePostText(postText = binding.etPost.text.toString(),
	...
	private fun handleEditPostSuccessResponse(post: AmityPost) {
		...
		resultIntent.putExtra(EXTRA_PARAM_POST_ID, post.getPostId())
