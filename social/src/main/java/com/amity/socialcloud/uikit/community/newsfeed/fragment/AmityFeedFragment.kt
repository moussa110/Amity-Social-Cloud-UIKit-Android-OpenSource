...
abstract class AmityFeedFragment : AmityBaseFragment() {
	...
	private fun handleLoadedState(itemCount: Int, loadStates: CombinedLoadStates) {
		...
		if (loadStates.source.refresh is LoadState.NotLoading && loadStates.append.endOfPaginationReached && itemCount < 1) {
			if (!emptyStatePublisher.hasComplete()) {
				emptyStatePublisher.onNext(true)
			}
		} else if (loadStates.source.refresh is LoadState.NotLoading && loadStates.append.endOfPaginationReached && itemCount > 0) {
			if (!emptyStatePublisher.hasComplete()) {
	...
	private fun observeRefreshEvents() {
		getViewModel().getRefreshEvents(onReceivedEvent = { refresh() }).untilLifecycleEnd(this)
			.subscribe()
	}

	private fun observeUserClickEvents() {
		getViewModel().getUserClickEvents(onReceivedEvent = {
			getViewModel().userClickListener.onClickUser(it)
		}).untilLifecycleEnd(this).subscribe()
	}

	private fun observeCommunityClickEvents() {
		getViewModel().getCommunityClickEvents(onReceivedEvent = {
			getViewModel().communityClickListener.onClickCommunity(it)
		}).untilLifecycleEnd(this).subscribe()
	}

	internal open fun observePostEngagementClickEvents() {
		getViewModel().getPostEngagementClickEvents(onReceivedEvent = {
			when (it) {
	...
	private fun observePostContentClickEvents() {
		getViewModel().getPostContentClickEvents(onReceivedEvent = {
			when (it) {
	...
	private fun navigateToStreamPlayer(stream: AmityStream) {
		if (stream.getStatus() == AmityStream.Status.LIVE || stream.getStatus() == AmityStream.Status.RECORDED) {
			val livestreamIntent =
				AmityLivestreamVideoPlayerActivity.newIntent(context = requireContext(),
	...
	private fun observeCommentEngagementClickEvents() {
		getViewModel().getCommentEngagementClickEvents(onReceivedEvent = {
			when (it) {
	...
	private fun observeCommentContentClickEvents() {
		getViewModel().getCommentContentClickEvents(onReceivedEvent = {
			when (it) {
	...
	private fun observeCommentOptionClickEvents() {
		getViewModel().getCommentOptionClickEvents(onReceivedEvent = {
	...
	internal fun showSharingOptions(post: AmityPost) {
		...
		bottomSheet.show(getViewModel().getSharingOptionMenuItems(post = post, shareToMyFeed = {
			...
			bottomSheet.dismiss()
			AmitySocialUISettings.postShareClickListener.shareToExternal(requireContext(), post)
		}))
	}
	...
	open fun deletePost(post: AmityPost) {
		getViewModel().deletePost(post = post, onSuccess = {
			// do nothing
		}, onError = {
			// display error
		}).untilLifecycleEnd(this).subscribe()
	...
	open fun closePoll(pollId: String) {
		getViewModel().closePoll(pollId = pollId, onSuccess = {
			// do nothing
		}, onError = {
			// display error
		}).untilLifecycleEnd(this).subscribe()
	...
	open fun approvePost(post: AmityPost) {
		getViewModel().approvePost(postId = post.getPostId(), onAlreadyApproved = {
	...
	open fun declinePost(post: AmityPost) {
		getViewModel().declinePost(postId = post.getPostId(), onAlreadyDeclined = {
			showAlreadyReviewedDialog()
		}, onError = {
			// display error
		}).untilLifecycleEnd(this).subscribe()
	...
	private fun showCommentOptions(comment: AmityComment) {
		val bottomSheet = AmityBottomSheetDialog(requireContext())
		bottomSheet.show(getViewModel().getCommentOptionMenuItems(comment = comment, editComment = {
			bottomSheet.dismiss()
			editComment(comment)
		}, deleteComment = {
			bottomSheet.dismiss()
	...
	internal open fun deleteComment(commentId: String) {
		getViewModel().deleteComment(commentId = commentId, onSuccess = {
			// do nothing
		}, onError = {
			// display error
		}).untilLifecycleEnd(this).subscribe()
