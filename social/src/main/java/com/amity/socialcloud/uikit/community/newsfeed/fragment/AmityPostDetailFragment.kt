...
class AmityPostDetailFragment : AmityBaseFragment(), SuggestionsVisibilityManager,
	...
	private fun observePost() {
		viewModel.getPost(postId = viewModel.postId, onLoaded = {
			...
			renderFooter(it)
		}, onError = {}).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread())
	...
	private fun renderFooter(post: AmityPost) {
		...
		isFirstLoad = false
		if (post.getFeedType() != AmityFeedType.REVIEWING && AmitySocialUISettings.getViewHolder(
				post.getType().getApiKey()).enableFooter()) {
	...
	private fun renderPost() {
		...
		val fragment =
			AmitySinglePostFragment.newInstance(viewModel.postId).postItemListener(postItemListener)
				.build(requireActivity() as AppCompatActivity)

		childFragmentManager.beginTransaction().replace(R.id.post_fragment_container, fragment)
			.commit()
	}
	...
	private fun renderComments(post: AmityPost) {
		...
		val fragment = AmityCommentListFragment.newInstance(post.getPostId())
			.readOnlyMode(viewModel.isPostReadOnly(post)).commentItemListener(commentItemListener)
			.build(requireActivity() as AppCompatActivity)

		childFragmentManager.beginTransaction()
			.replace(R.id.comment_list_fragment_container, fragment).commit()
	}

	fun isJoined(post: AmityPost): Boolean {
		return (post.getTarget() as? AmityPost.Target.COMMUNITY)?.getCommunity()?.isJoined() ?: true
	}
	...
	private fun setupComposeBar(post: AmityPost) {
		...
		binding.commentComposeBar.setCommentExpandClickListener {
			...
			bundle.putString(EXTRA_PARAM_COMMENT_TEXT,
	...
	private fun showReplyTo() {
		...
		binding.textviewReplyTo.startAnimation(AnimationUtils.loadAnimation(context,
	...
	private fun showPostOptions() {
		viewModel.post?.let {
			...
			bottomSheet.show(viewModel.getPostOptionMenuItems(post = it, editPost = {
				bottomSheet.dismiss()
				editPost(it)
			}, deletePost = {
				bottomSheet.dismiss()
	...
	private fun deletePost(post: AmityPost) {
		viewModel.deletePost(post = post, onSuccess = {
			backPressFragment()
		}, onError = {
	...
	private fun closePoll(pollId: String) {
		viewModel.closePoll(pollId = pollId, onSuccess = {
			// do nothing
		}, onError = {
			// display error
		}).untilLifecycleEnd(this).subscribe()
