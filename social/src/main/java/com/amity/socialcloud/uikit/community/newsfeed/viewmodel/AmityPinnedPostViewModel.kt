...
class AmityPinnedPostViewModel(private val savedState: SavedStateHandle) : AmityFeedViewModel() {
    ...
    private fun checkReviewPermission(post: AmityPost): Flowable<Boolean> {
        ...
        return if (target is AmityPost.Target.COMMUNITY && target.getCommunity() != null) {
            ...
            hasCommunityPermission(Flowable.just(community),
                getCommunityPermissionSource(
                    community.getCommunityId(), AmityPermission.REVIEW_COMMUNITY_POST
                )
            )
        } else {
	        Flowable.just(false)
