...
class AmityCreatePostViewModel : AmityBaseViewModel() {
	...
	private fun setupImagePost(postChildren: List<AmityPost>) {
		postChildren.map { getPostImage(it) }.forEach { image ->
			image?.let {
				val feedImage = mapImageToFeedImage(it)
				imageMap[feedImage.url.toString()] = feedImage
			}
	...
	private fun setupVideoPost(postChildren: List<AmityPost>) {
		postChildren.map { getPostVideoThumbnail(it) }.forEach { image ->
			image?.let {
	...
	private fun setupFilePost(postChildren: List<AmityPost>) {
		postChildren.map { getPostFile(it) }.forEach { file ->
			file?.let {
				val attachment = mapFileToFileAttachment(it)
				filesMap[attachment.uri.toString()] = attachment
			}
	...
	private fun mapFileToFileAttachment(ekoFile: AmityFile): AmityFileAttachment {
		...
		return AmityFileAttachment(ekoFile.getFileId(),
			null,
			ekoFile.getFileName(),
			fileSize,
	...
	private fun mapFileToFileAttachment(fileAttachment: AmityFileAttachment,
	                                    ekoFile: AmityFile): AmityFileAttachment {
		val fileSize = ekoFile.getFileSize().toLong()
		return AmityFileAttachment(ekoFile.getFileId(),
	...
	fun observeCommunity(communityId: String): Completable {
		return communityRepository.getCommunity(communityId).subscribeOn(Schedulers.io())
			.observeOn(AndroidSchedulers.mainThread()).doOnNext {
				this.community = it
			}.ignoreElements()
	}
	...
	fun getPostDetails(postId: String,
	                   ...
	                   onPostLoadFailed: (Throwable) -> Unit): Completable {
		return postRepository.getPost(postId).firstOrError().subscribeOn(Schedulers.io())
	...
	fun updatePostText(postText: String,
	                   ...
	                   onUpdateFailed: (Throwable) -> Unit): Completable {
