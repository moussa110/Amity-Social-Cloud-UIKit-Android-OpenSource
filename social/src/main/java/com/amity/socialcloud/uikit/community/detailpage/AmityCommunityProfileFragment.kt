...
class AmityCommunityProfileFragment : RxFragment() {
	...
	private fun initMoreIconInActionBar() {
		(requireActivity() as AmityBaseToolbarFragmentContainerActivity).getToolBar()?.let {
			it.setRightDrawable(if (shouldShowOptionsMenu) ContextCompat.getDrawable(requireActivity(),
				R.drawable.amity_ic_more_horiz) else null)
	...
	/*override fun onCreateOptionsMenu(menu: Menu, inflater: MenuInflater) {
		if (shouldShowOptionsMenu) {
			...
			drawable?.colorFilter =
				BlendModeColorFilterCompat.createBlendModeColorFilterCompat(R.color.amityColorBlack,
	...
	private fun navigateToCommunitySettings() {
		val intent = AmityCommunitySettingsActivity.newIntent(requireContext(), viewModel.communityId)
	...
	private fun navigateToEditProfile() {
		val intent =
			AmityCommunityProfileActivity.newIntent(requireContext(), viewModel.communityId!!)
		startActivity(intent)
	}

	private fun navigateToMembersPage() {
		val intent = AmityCommunityMemberSettingsActivity.newIntent(requireContext(),
	...
	private fun observeCommunity() {
		viewModel.observeCommunity(onLoaded = {
	...
	private fun renderCommunityData(community: AmityCommunity) {
		...
		binding.tvName.setCompoundDrawablesWithIntrinsicBounds(leftDrawable,
			null,
			rightDrawable,
			null)
		...
		community.getPostCount(AmityFeedType.PUBLISHED).subscribeOn(Schedulers.io())
			.observeOn(AndroidSchedulers.mainThread()).doOnNext {
				binding.tvPostCount.text = it.toDouble().formatCount()
			}.doOnError {
				Timber.e(it)
			}.untilLifecycleEnd(this, communityProfileComposer).subscribe()
	...
	private fun renderPostReviewBanner(data: PostReviewBannerData) {
		if (data.isVisible) {
			...
			if (data.isReviewer) {
				val reviewerMessage =
					binding.root.resources.getQuantityString(R.plurals.amity_community_banner_number_of_pending_posts,
						data.postCount,
